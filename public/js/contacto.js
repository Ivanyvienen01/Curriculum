        ...
        con.connect(function(error){
            try{
                if(error){
                    ...
                    con.query(`SELECT COUNT(*) AS usuario FROM USUARIOS WHERE correo = '${email}'`,function(error, res, campo){
                        if(error){
                            console.log("Error en select BD -- " + console.error());
                        }else{
                            console.log("Usuarios encontrados: " + res[0].usuario);
                            if(parseInt(res[0].usuario) == 0){
                                con.query(`INSERT INTO usuarios (nombre, correo) values('${name}','${email}')`
